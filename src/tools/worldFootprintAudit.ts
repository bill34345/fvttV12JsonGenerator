...
export async function runWorldFootprintAudit(
  ...
  const baseline = resolved.baselineFile
    ? await readBaseline(resolved.baselineFile)
    : createPendingBaseline();
  ...
  for (const [name, value] of Object.entries(deliverables)) {
    ...
    await writeFile(join(resolved.outputDir, name), contents, "utf8");
  ...
  await writeFile(
    join(resolved.outputDir, "audit-manifest.json"),
    stableJson(manifest),
    "utf8",
  );
  return manifest;
...
async function readBaseline(path: string): Promise<AuditBaseline> {
  const baseline = await readJsonRecord(path, "baseline");
  if (typeof baseline.status !== "string" || baseline.status.length === 0) {
    throw new Error("Baseline file must contain a non-empty status");
  }
  return baseline as AuditBaseline;
}
