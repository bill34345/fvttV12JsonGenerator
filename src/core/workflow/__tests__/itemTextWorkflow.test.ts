...
describe("ItemTextWorkflow", () => {
  ...
  it("does not regenerate or remove unrelated vault inputs during an item-only run", async () => {
    const root = mkdtempSync(join(tmpdir(), "fvtt-item-workflow-scope-"));
    const sourcePath = join(root, "items.md");
    const vaultPath = join(root, "vault");
    const inputDir = join(vaultPath, "input");
    const outputDir = join(vaultPath, "output");
    const unrelatedOutputPath = join(outputDir, "unrelated.json");

    mkdirSync(inputDir, { recursive: true });
    mkdirSync(outputDir, { recursive: true });
    writeFileSync(
      join(inputDir, "unrelated.md"),
      ["---", "名称: 不相关角色", "类型: npc", "---", "# 不相关角色"].join("\n"),
      "utf-8",
    );
    writeFileSync(unrelatedOutputPath, '{"sentinel":"keep"}', "utf-8");
    writeFileSync(
      sourcePath,
      [
        "# 下面是一个示例物品",
    ...
    try {
      const workflow = new ItemTextWorkflow();
      const result = await workflow.run({ sourcePath, vaultPath, fvttVersion: "14" });

      expect(result.sync.processed).toBe(1);
      expect(readFileSync(unrelatedOutputPath, "utf-8")).toBe('{"sentinel":"keep"}');
      expect(existsSync(join(outputDir, "items", "shield-of-the-cavalier__骑士之盾.json"))).toBe(true);
    } finally {
      rmSync(root, { recursive: true, force: true });
    }
  });
