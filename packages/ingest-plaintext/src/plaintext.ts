...
export interface PlainTextHttpRequest {
  method: 'POST';
...
export interface PlainTextHttpResponse {
  ok: boolean;
  status: number;
  json(): Promise<unknown>;
}

export interface PlainTextTranslationService {
  translate(
    text: string,
    context: {
...
export interface PlainTextTranslationFactoryOptions {
  apiKey: string;
  baseUrl: string;
  model: string;
  timeoutMs: number;
  httpClient: (
    url: string,
    init: PlainTextHttpRequest,
  ) => Promise<PlainTextHttpResponse>;
}

export type PlainTextTranslationFactory = (
  options: PlainTextTranslationFactoryOptions,
) => PlainTextTranslationService;

export class PromptedIngestNormalizer implements PlainTextAiNormalizer {
  private readonly translator: PlainTextTranslationService;
  ...
  constructor(options: {
    ...
    model: string;
    timeoutMs: number;
    translationFactory: PlainTextTranslationFactory;
  }) {
    this.translator = options.translationFactory({
