...
import type {
  CollectionIngestionPort,
  ItemCollectionBlockPort,
} from './externalPorts';
import type {
  IconWorkflowOptions,
  IconWorkflowPort,
...
export interface CollectionConversionDependencies {
...
export async function convertMonsterCollectionToJson(
  ...
  dependencies: CollectionConversionDependencies,
): Promise<CollectionConversionResult> {
  const blocks = dependencies.ingestion.splitMonsterCollection(options.content);
  if (blocks.length === 0) {
  ...
  for (let index = 0; index < blocks.length; index++) {
    ...
    try {
      const generated = dependencies.ingestion.parseMonsterBlock(block.rawBlock);
      ...
      const result = await convertMarkdownContentToJson({
        ...
        iconWorkflow: dependencies.iconWorkflow,
      });
  ...
  return summarizeCollection(
    'monster-collection',
    items,
    options.fvttVersion ?? '12',
    options.effectProfile ?? 'core',
    outputDir,
    dependencies.iconWorkflow,
  );
}
...
export async function convertItemCollectionToJson(
  ...
  const blocks = dependencies.ingestion.splitItemCollection(options.content);
  if (blocks.length === 0) {
...
function summarizeCollection(
  ...
  if (iconReview) {
    ...
    iconWorkflow.writeReviewReport(path, iconReview);
    outputFiles.push({
      id: 'icon-review.json',
...
function itemBlockToProjectMarkdown(block: ItemCollectionBlockPort): string {
...
function itemBlockFileName(block: ItemCollectionBlockPort): string {
