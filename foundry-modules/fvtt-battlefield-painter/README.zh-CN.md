# Battlefield Painter P1 Alpha · `0.2.0-alpha.1`
...
当前暂不包含音效、WebM 动画、12 色变体、跨客户端并发锁、伤害自动结算和第三方自动化模组集成。
...
P1 alpha 还提供仅供开发、测试和修复使用的内部阶段开关：

```js
const api = game.modules.get("fvtt-battlefield-painter").api
api.developmentPhases()
api.setDevelopmentPhase("p1", false) // 回退为 P0 单格自由画笔
