...
export class PainterController {
  ...
  activate(): void {
    ...
    this.#update({ active: true, status: "画笔已启用：在地图上按住并拖动。" });
  }
  ...
  async #commitStroke(): Promise<void> {
    ...
    try {
      ...
      if (this.#state.mode === "paint") {
        const result = await service.paintCells(
        ...
        await service.eraseCellKeys(new Set(cells.map(({ key }) => key)));
        this.#update({ status: `已擦除命中的地形格位。` });
      } else {
        await service.advanceCellKeys(new Set(cells.map(({ key }) => key)));
