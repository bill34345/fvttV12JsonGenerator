...
export function spellResolverPaths(config: FoundryLabConfig): SpellResolverPaths {
  ...
  return {
    buildDir: resolve(config.repoRoot, 'foundry-modules/monster-spell-resolver/dist/module'),
    zipPath: resolve(
      config.repoRoot,
      'foundry-modules/monster-spell-resolver/dist',
      `${SPELL_RESOLVER_MODULE_ID}.zip`,
    ),
    modulesRoot,
...
export async function buildSpellResolverForLab(
  ...
  assertExactRepoPath(config, paths.zipPath, [
    'foundry-modules',
    'monster-spell-resolver',
    'dist',
    `${SPELL_RESOLVER_MODULE_ID}.zip`,
  ], 'Spell resolver ZIP path');
...
function assertExactBuildDirectory(config: FoundryLabConfig, target: string): void {
  const expected = resolve(config.repoRoot, 'foundry-modules/monster-spell-resolver/dist/module');
  if (resolve(target) !== expected || !isAbsolute(target) || relative(config.repoRoot, target).startsWith('..')) {
    throw new Error(`Spell resolver build directory must be the exact module dist path: ${expected}`);
  }
  assertExactRepoPath(config, target, [
    ...
    'dist',
    'module',
  ], 'Spell resolver build directory');
}
