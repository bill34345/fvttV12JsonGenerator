...
async function buildBrowserBundleInSubprocess(paths: SpellResolverBuildPaths): Promise<void> {
  const child = Bun.spawn(
    [process.execPath, resolve(paths.packageRoot, 'build.ts'), '--bundle-only', paths.repoRoot],
    {
      cwd: paths.repoRoot,
      stdout: 'pipe',
      stderr: 'pipe',
...
export function assertSpellResolverBuildMutationBoundary(repoRoot: string): SpellResolverBuildPaths {
  ...
  const packageRoot = resolve(root, 'foundry-modules/monster-spell-resolver');
  const paths: SpellResolverBuildPaths = {
    repoRoot: root,
    packageRoot,
    sourceRoot: resolve(packageRoot, 'src'),
    distRoot: resolve(packageRoot, 'dist'),
    outputDir: resolve(packageRoot, 'dist/module'),
    zipPath: resolve(packageRoot, 'dist', `${MODULE_ID}.zip`),
  };
