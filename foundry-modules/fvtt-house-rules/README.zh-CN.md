...
| HR-04 自然 20 | 武器攻击与进行攻击检定的法术都会先按 dnd5e 原生规则生成完整重击骰池，再把第一伤害部分的第一颗骰拆成 `1dXminX`，因此原始骰面仍会投出而最终按满值结算：`1d8` 重击显示为 `1d8min8 + 1d8`，`2d6` 重击显示为 `1d6min6 + 3d6`。其他骰、其他伤害部分和 rider 继续使用原生重击；无自然 20 攻击消息、目标伤害部分不唯一或骰子结构无法安全识别时失败关闭。 |
