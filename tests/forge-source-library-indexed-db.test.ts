...
describe('Forge IndexedDB source library adapter', () => {
  test('commits a strict envelope, reloads it, and rejects a stale optimistic revision', async () => {
    const factory = new FakeIndexedDbFactory();
    const store = new IndexedDbForgeSourceLibraryStore('world:user', factory as unknown as IDBFactory, undefined);
    const first = stateAt(1);
    await store.replace(0, first);
    expect(await store.load()).toEqual(first);
    await expect(store.replace(0, stateAt(2))).rejects.toThrow(ForgeSourceLibraryConflictError);
    expect((await store.load()).revision).toBe(1);
    expect(factory.openDatabases).toBe(factory.closedDatabases);
  });

  test('waits for transaction failure, maps quota errors, and preserves the committed envelope', async () => {
    const factory = new FakeIndexedDbFactory();
    const store = new IndexedDbForgeSourceLibraryStore('world:user', factory as unknown as IDBFactory, undefined);
    await store.replace(0, stateAt(1));
    factory.failNextPutWithQuota = true;
    await expect(store.replace(1, stateAt(2))).rejects.toThrow(/quota.*previous library is unchanged/iu);
    expect((await store.load()).revision).toBe(1);
  });

  test('aborts when authority changes during the IndexedDB read and before put', async () => {
    const factory = new FakeIndexedDbFactory();
    const store = new IndexedDbForgeSourceLibraryStore('world:user', factory as unknown as IDBFactory, undefined);
    await store.replace(0, stateAt(1));
    let authorized = true;
    ...
    await expect(store.replace(1, stateAt(2), () => {
      if (!authorized) throw new Error('GM authority changed during IndexedDB transaction.');
    })).rejects.toThrow(/authority changed/u);
    expect((await store.load()).revision).toBe(1);
  });

  test('notifies local and same-scope peers only, then suppresses all notifications after close', async () => {
    const factory = new FakeIndexedDbFactory();
    const channels = new FakeChannelHub();
    const first = new IndexedDbForgeSourceLibraryStore('world:user', factory as unknown as IDBFactory, channels.create);
    const peer = new IndexedDbForgeSourceLibraryStore('world:user', factory as unknown as IDBFactory, channels.create);
    const other = new IndexedDbForgeSourceLibraryStore('world:other', factory as unknown as IDBFactory, channels.create);
    let localNotices = 0;
    let peerNotices = 0;
    let otherNotices = 0;
    first.subscribe(() => { localNotices += 1; });
    peer.subscribe(() => { peerNotices += 1; });
    other.subscribe(() => { otherNotices += 1; });
    await first.replace(0, stateAt(1));
    await tick();
    expect({ localNotices, peerNotices, otherNotices }).toEqual({ localNotices: 1, peerNotices: 1, otherNotices: 0 });
    ...
    await tick();
    expect({ localNotices, peerNotices, otherNotices }).toEqual({ localNotices: 1, peerNotices: 1, otherNotices: 0 });
    expect(channels.postAfterClose).toBe(0);
  });
});

function stateAt(revision: number): ForgeSourceLibraryV1 {
  return {
    ...emptyForgeSourceLibrary(),
    revision,
    updatedAt: `2026-08-30T08:0${revision}:00.000Z`,
  };
}
  ...
  objectStore(): IDBObjectStore {
    return {
      ...
      put: (value: string, key: IDBValidKey) => this.request(() => {
        ...
        if (this.owner.failNextPutWithQuota) {
          this.owner.failNextPutWithQuota = false;
  ...
  private request<T>(operation: () => T): IDBRequest<T> {
    ...
    queueMicrotask(() => {
      ...
      try {
        ...
        this.error = error instanceof DOMException ? error : new DOMException(String(error), 'UnknownError');
        this.active = false;
...
class FakeChannelHub {
  ...
  broadcast(sender: FakeChannel, data: unknown): void {
    if (sender.closed) {
      this.postAfterClose += 1;
      throw new DOMException('BroadcastChannel is closed.', 'InvalidStateError');
    }
    for (const channel of this.channels) {
      if (channel !== sender && !channel.closed && channel.name === sender.name) {
