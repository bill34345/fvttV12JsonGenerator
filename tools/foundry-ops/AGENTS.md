...
- `offline-world mutation`：只处理停止状态、精确授权的离线世界副本，不代表修改生产；不创建备份。
...
- 世界写操作要求精确世界身份和当次明确授权；不直接编辑运行中的 LevelDB，失败时停止并报告，不创建备份或回滚副本。
...
- 真实操作有精确路径和事后状态证据；生产只读记录机械门禁与读取范围，任何写入另有当次授权和核对证据，不创建备份。
- 明确区分“生成计划”“完成复制”“完成切换”“删除旧副本”“生产验收”，不得用前一步替代后一步。

## 用户明确授权的无备份运维规则（2026-08-13）

- 用户明确要求本地和线上任何任务都不创建备份，也不把在线人数为零作为已授权生产操作的前置条件。报告必须记录实际目标、文件清单、SHA-256、进程、端口、世界和日志结果。
- 该规则不扩大授权：不得扩展到未授权目标、运行中的 LevelDB 直接编辑或 Foundry/dnd5e 升级；生产写入仍需要当次明确授权和精确目标核对。
- 不保留旧副本，不使用隐藏目录、临时复制或回滚目录变相备份。失败时停止并报告，等待新的明确修复授权。
