...
| PROD-001 | P2 | in_progress | No source-derived DAE-only fixture exists, so DAE behavior beyond coexistence is not accepted. | Generalized fixture generated through CLI and exercised in locked DAE 14.0.12 runtime with source-to-output review. |
...
- [ ] (2026-07-15) M9 Task 3 started after the Task 2 runtime dependency was isolated; PROD-001 moved to `in_progress` before adding a source-duration hint or DAE mapping.
...
**Exact next action:** Execute M9 Task 3 RED with three source clauses for an inflicted condition lasting until the target takes damage, one close negative, and one unrelated Actor; inspect the exact DAE 14.0.12 `isDamaged` registration/expiry paths again before implementing the neutral source-duration hint and `modded-v14` mapping.
...
- Decision: Treat process-local request/job controls as bounded single-process defenses, not as a substitute for reverse-proxy user authentication, TLS, firewalling, or operating-system limits.
  ...
  Date/Author: 2026-07-15, Codex.
- Decision: Order Milestone 9 as standalone Item correctness/runtime, DAE-only behavior/runtime, corpus expansion, then documentation reconciliation.
