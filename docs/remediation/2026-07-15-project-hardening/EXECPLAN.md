...
| PROD-001 | P2 | in_progress | Source-derived DAE/MIDI generation now passes code and CLI acceptance, but real damage-triggered effect removal has not been exercised because the browser file-upload permission remains disabled. | Generalized fixture generated through CLI and exercised in the locked DAE 14.0.12 + MIDI-QOL 14.0.9 runtime with source-to-output review. |
| PROD-002 | P2 | in_progress | Standalone Item v14 live Foundry acceptance is absent. | Real source item generated by CLI, imported, opened, exercised, re-exported, and semantically compared. |
...
- [ ] (2026-07-15) M9 Task 2 runtime preflight reached the matrix world's Foundry Import Data dialog, but the Chrome extension rejected local file selection until its user-controlled "Allow access to file URLs" permission is enabled. The local server was stopped, port released, and world option restored; PROD-002/SEM-005 remain open with exact resume state recorded in the child plan.
...
**Exact next action:** Resume M9 Task 2 and Task 3 after the user enables Chrome extension **Allow access to file URLs**: import the existing Shield and Damage-Bound Warden artifacts through Foundry UI in `fvtt-v14-module-matrix`, complete Item equip/Activity/export semantics and DAE+MIDI damage-removal/control semantics, clean disposable documents, stop the server, and restore options. Authenticated crawling still requires separate explicit credential/session authorization.
...
- Observation: Local Foundry itself accepted the matrix-world login and exposed the correct Item import workflow, but the Chrome extension could not supply the CLI JSON because its user-controlled file-URL permission is disabled.
