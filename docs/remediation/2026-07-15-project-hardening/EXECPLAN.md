...
| SPELL-002 | P1 | in_progress | The first user-run online Rat Warlock hydration reached the eighth deterministic Activity (`116c319da6fcfdd2`, Faerie Fire), observed a public `createItem` cache with the correct Actor/`cachedFor`/Compendium identity but a mismatched complete prepared-getter projection, timed out, and then reported rollback residual differences. The Actor visibly retained only partial managed spell content and explicit re-resolution entered manual-conflict review with Apply disabled. | Export and inspect the resolver diagnostic JSON before another write; capture exact Foundry/dnd5e/resolver and active-module versions; identify the expected-versus-actual Faerie Fire projection path and rollback residual path; add a minimal failing test; implement one root-cause fix; rerun focused/aggregate gates plus authorized online hydration, full rollback, retry, and ten-spell semantic acceptance. Do not close from a successful retry alone. |
| SPELL-003 | P1 | in_progress | After the persisted online hydration state became `failed-recovery-required`, explicit re-resolution set an ephemeral `needs_review` state. The public status action and exported diagnostic top-level `status` then reported `needs_review` while `spellResolution.status` still correctly recorded the more severe recovery failure. | Failure/recovery-required persisted status must outrank later non-active ephemeral review states; add focused status, report, diagnostics, dialog-close, and re-resolution regressions; verify the Actor keeps a visible recovery-required marker until an authorized recovery actually succeeds. |
