...
| SEM-005 | P1 | in_progress | The former standalone Item template leakage and omitted Shield mechanics are repaired and now pass live import/equip/Activity/readback semantics; the supported UI export/download artifact was not captured, so the required re-export comparison remains open. | Generalized armor/action schema parsing backed by locked dnd5e 4.3.9 and 5.3.3 references; positive/negative/unrelated fixtures; v12/v14 CLI regeneration; the PROD-002 local Foundry import/open/exercise/re-export comparison passes without template leakage or omitted source mechanics. |
...
| PROD-002 | P2 | in_progress | The real Shield passes UI import, sheet/readback, equipped+attuned AC `10 -> 12 -> 14`, Forceful Bash attack/damage/prone semantics, and one-per-dawn Protective Field depletion/concentration semantics; a UI export/download file was not captured for the final artifact comparison. | Real source item generated by CLI, imported, opened, exercised, re-exported, and semantically compared. |
...
- [ ] (2026-07-15) M9 Task 2 runtime semantics passed after the Chrome permission change: CLI v14 Shield UI import/readback, AC `10 -> 12 -> 14`, Forceful Bash hit/damage/prone handling, and Protective Field one-per-dawn depletion/concentration all behaved as sourced. PROD-002/SEM-005 remain open only because the UI export event timed out and no downloaded re-export artifact was captured for final comparison.
...
**Exact next action:** Resume only the remaining M9 Task 2 export proof in `fvtt-v14-module-matrix`: import the existing CLI Shield through Foundry UI, capture its supported Export Data download, compare source-relevant fields to the CLI artifact while classifying migration-only volatility, delete disposable documents, stop the server, and restore options. Authenticated crawling still requires separate explicit credential/session authorization; exact production coexistence still requires a valid authorized protected package set or explicit acceptance as remaining scope.
...
- [ ] Run `bun run web:build` and the browser upload/download smoke workflow. The build passes; gstack's separate browser smoke remains unaccepted because proactive use is disabled and the installed `browse.exe` cannot resolve its packaged `server.ts`. Chrome file access is now enabled and no longer blocks local Foundry import.
...
- [ ] Run the authorized local Foundry runtime matrix and preserve the boundary between minimal runtime, copied-world, and production claims. Shield behavior/readback and the locked modded/core `isDamaged` control now pass; the Shield UI export artifact comparison remains open, and exact production coexistence remains external.
