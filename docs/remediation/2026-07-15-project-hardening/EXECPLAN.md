...
**Exact next action:** Create the focused Milestone 9 child plan by inventorying the current acceptance reports, locked local Foundry v14/DAE runtime helpers, the standalone Shield source/output path, and the existing corpus controls; then repair SEM-005/PROD-002 through a generalized TDD cycle before reconciling the support matrix.
