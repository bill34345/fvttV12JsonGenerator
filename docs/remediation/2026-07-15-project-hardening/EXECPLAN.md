...
| WEB-001 | P1 if public, P3 if loopback-only | in_progress | Web API advertises public unauthenticated access while exposing expensive jobs and server-side configured capabilities. | Deployment mode explicitly chosen; secure default binding/auth policy implemented and tested; docs match actual exposure. |
| WEB-002 | P1 if proxied | in_progress | Client identity trusts forwarded headers without a trusted-proxy boundary; in-memory per-IP limiting can be spoofed or collapse clients. | Trusted proxy configuration, fallback identity behavior, global caps, and tests for forged headers and proxy chains. |
| WEB-003 | P2 | in_progress | Upload limits are checked after `request.text()` and process-local job/rate state is not a complete public resource-control boundary. | Request-body/server limits, global job cap, cleanup bounds, and load/abuse tests are documented and enforced for public deployment. |
...
**Exact next action:** Execute M8 Task 1 RED by adding focused configuration/API tests for loopback defaults, forbidden implicit non-loopback exposure, explicit public-mode token requirements, and unauthenticated API rejection before implementing the security config module.
