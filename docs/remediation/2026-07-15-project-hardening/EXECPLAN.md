...
| CLASSPACK-001 | P1 | closed | Local `dnd5e_classpack` 4.3.4 declared Foundry maximum 13, its remote manifest/download metadata caused dnd5e 5.3.3 to skip automatic module-compendium migration, and six shipped macros called the removed `Actor#rollAbilitySave` API. | Closed at the local-module boundary by the guarded `4.3.4-v14.2` fork, locked dnd5e/DAE migration, 12 exact-hash macro patches, an exact 21-pack identity gate, 5,655-document strict DataModel validation, real array-returning saving-throw execution, repeat/no-write verification, and a module-disabled world-launch control. The pre-commit v14.2 hardening adds fail-closed Foundry/dnd5e/active-DAE version gates and deterministic LF comparison without changing pack/Macro data. Production remains on separately accepted v14.1 load/index evidence. Full interactive target-selection, movement, damage, and MIDI gameplay branches remain explicitly unclaimed. |
...
### 2026-07-29 Local dnd5e Classpack v14 Repair

- [x] Confirmed the authorized scope is only `.local/foundry-v14/data/server-mirror/Data/modules/dnd5e_classpack`; production and `cor-cotn` world data are outside scope.
- [x] Confirmed the upstream content-only 4.3.4 module has 21 LevelDB packs, preserves externally referenced Compendium UUIDs, declares maximum Foundry 13, and is not currently enabled in the disposable matrix world.
- [x] Confirmed locked dnd5e 5.3.3 skips module-owned compendiums carrying `manifest` or `download`, while its public compendium migrator covers Actor/Item/Scene documents only; JournalEntry, RollTable, and Macro require separate validation.
- [x] Confirmed the macro pack contains 12 script macros; six use removed `rollAbilitySave`, with additional legacy `.data`, `duplicate()`, MIDI-QOL, and DAE call sites requiring exact-version review.
- [x] User explicitly declined a backup for this local module. Recovery is limited to disabling the module and reinstalling upstream 4.3.4; the workflow must not imply that disabling reverses already migrated pack bytes.
- [x] Implemented and tested the guarded/idempotent Foundry Lab migration in `milestones/14-classpack-v14-migration.md`.
- [x] Applied it directly to the local module. All package, pack, top-level document, and non-duplicate embedded identities remained stable; dnd5e 5.3.3 removed exactly one algorithm-confirmed duplicate transferred Invisible effect (`summons` Actor `4XOCbibDwZcdgbhV`, ActiveEffect `dnd5einvisible00`).
- [x] Enabled only in `fvtt-v14-module-matrix` and completed Foundry 14.364 / dnd5e 5.3.3 data plus representative feature acceptance. The final state leaves classpack enabled and MIDI-QOL disabled in the disposable matrix, restores the configured default world to `cor-cotn`, and leaves Foundry stopped.
- [x] Final evidence: 21 packs, 5,655 top-level documents, 13,302 runtime document identities, 13,558 LevelDB records, 12 patched/compiled Macros with zero forbidden markers, 5,655 strict DataModel validations with zero failures, a real Constitution save returning one evaluated `D20Roll`, 197 Foundry Lab tests, both TypeScript configurations, and `git diff --check`.
- [x] The module-disabled control reached `Launching World | Complete` and listened on port 30001. Disabling is therefore a valid load recovery control, but it does not reverse the already migrated compendium bytes.
- [x] Remaining acceptance boundary: browser navigation policy prevented a fresh post-restart MIDI-QOL client smoke. Exact local MIDI-QOL 14.0.11 and DAE 14.0.12 sources were used for the patch, but interactive target selection, token movement, damage application, and every macro branch were not gameplay-executed and are not claimed.
- [x] Pre-commit review rejected the initial landing because Windows CRLF made a no-write dry-run report `changed: true`, and the module did not enforce its dnd5e/active-DAE runtime boundary. Local `4.3.4-v14.2` now normalizes tracked runtime text to LF, pins Foundry 14.364 / dnd5e 5.3.3 / MIDI-QOL 14.0.11 / DAE 14.0.12, refuses migration under another active runtime, and validates the exact 21-pack surface.
- [x] Applied only the v14.2 manifest/runtime hardening to the stopped local mirror. The 12 Macro hashes, 13,558 LevelDB records, and identity SHA-256 `1ceed323d42a302c0bc9d7b3c380516e5254b57382c1ad8c2ace06ea11b0055d` remained unchanged; the immediate second dry-run returned `changed: false`. Production 8080 remains v14.1 and was not accessed or changed.
...
2026-07-31: Prepared the standalone dnd5e classpack v14 workflow for a scope-separated `master` commit. Pre-commit semantic review rejected the original v14.1 landing because Windows CRLF made its no-write runtime comparison falsely report drift and because dnd5e/active-DAE version changes were documented but not enforced. Local v14.2 now normalizes runtime text, pins the exact Foundry 14.364 / dnd5e 5.3.3 / MIDI-QOL 14.0.11 / DAE 14.0.12 contract, refuses migration outside it, and locks all 21 pack identities. Applying v14.2 to the stopped local mirror changed no Macro or pack data; the 13,558 LevelDB records and identity SHA-256 `1ceed323d42a302c0bc9d7b3c380516e5254b57382c1ad8c2ace06ea11b0055d` remained stable and the second dry-run returned `changed: false`. Final gates pass 13 focused, 198 Foundry Lab, and 1,576 repository tests with 7,451 assertions plus coverage, typechecks, anti-overfit, hygiene, locked references, Web build, offline smoke, and diff checking. Production 8080 remains on v14.1; no production access or deployment occurred.
