...
**Exact next action:** Execute M9 Task 1 RED by extending `item-source-semantics.test.ts` with neutral-template, explicit shield/extra-AC, duration/concentration/aura, conditional-prone, close-negative, and unrelated structural expectations before changing Item parser/generator code.
