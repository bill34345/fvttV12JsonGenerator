...
describe('Web AI connection configuration', () => {
  ...
  it('requires an explicit environment opt-in before exposing the Companion pairing endpoint', () => {
    ...
    expect(getWebSecurityConfig(publicBase).aiConnections.companionEnabled).toBe(false);
    expect(getWebSecurityConfig({ ...publicBase, FVTT_WEB_CODEX_COMPANION_ENABLED: '1' }).aiConnections.companionEnabled).toBe(true);
  });
});
