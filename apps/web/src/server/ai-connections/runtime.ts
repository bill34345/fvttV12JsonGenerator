...
export interface AiConnectionsOverview {
  ...
  companion: {
    available: boolean;
    platform: 'windows';
    defaultModel: typeof COMPANION_DEFAULT_MODEL;
    ...
    controlUrl: string;
    controlProtocolVersion: typeof COMPANION_CONTROL_PROTOCOL_VERSION;
  };
    ...
    async handleApiRequest(request, clientIp, maxBodyBytes) {
      ...
      try {
        ...
        if (request.method === 'POST' && url.pathname === '/api/ai-connections/codex/pairings') {
          ...
          const model = optionalString(body.model, 256) ?? COMPANION_DEFAULT_MODEL;
      ...
      function overview(session: AnonymousSession): AiConnectionsOverview {
        return {
          ...
          companion: {
            ...
            platform: 'windows',
            defaultModel: COMPANION_DEFAULT_MODEL,
            defaultReasoningEffort: 'xhigh',
