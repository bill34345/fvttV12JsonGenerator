...
export function App() {
  ...
  const [companionControlCredential, setCompanionControlCredential] = useState<string | null>(null);
  ...
  async function connectCodexCompanion(input: {
    ...
    setAiPanelMessage("");
    setCompanionControlError("");
    setCompanionControlCredential(null);
    const remotePage = companionControlClient.isRemotePage();
    let confirmationWindow: Window | undefined;
    try {
      if (!remotePage && !companionControlClient.isLocalDevelopmentPage()) {
        throw new CompanionControlError(
          "COMPANION_REMOTE_HTTPS_REQUIRED",
          "本机 Companion 只支持本机开发页，或使用 HTTPS 的远程网页。",
        );
      }
      // Open the blank window while the click still has browser user activation.
      if (remotePage) confirmationWindow = companionControlClient.openRemoteConfirmationWindow();
      ...
      try {
        const remoteApproval = remotePage
          ? await companionControlClient.waitForRemoteApproval(controlUrl, pairing.id, confirmationWindow!)
          : undefined;
        const health = remoteApproval
          ? undefined
          : companionHealth ?? await companionControlClient.health(controlUrl);
        if (health) setCompanionHealth(health);
        const paired = await companionControlClient.pair(controlUrl, {
          instanceId: remoteApproval?.instanceId ?? health!.instanceId,
          pairingId: pairing.id,
          pairingToken: token,
          ...(remoteApproval ? {
            approvalId: remoteApproval.approvalId,
            pairAuthorization: remoteApproval.pairAuthorization,
          } : {}),
        });
        setCompanionControlCredential(paired.controlCredential);
        setCompanionHealth((current) => current && current.instanceId === paired.instanceId
          ? { ...current, status: "connecting", diagnostic: undefined }
          : {
              protocolVersion: overview.companion.controlProtocolVersion,
              service: "fvtt-ai-companion",
              version: remotePage ? "remote-approved" : "unknown",
              instanceId: paired.instanceId,
              status: "connecting",
            });
        setAiPanelMessage(remotePage
          ? "你已在本机确认这个网站；一次性配对已交给 Companion，正在完成安全检查。"
          : "已将一次性配对交给本机 Companion，页面会自动等待安全门禁完成。");
      } catch (controlError) {
  ...
  async function shutdownCompanion() {
    if (!aiOverview?.companion.controlUrl || !companionHealth || !companionControlCredential) {
      setAiPanelError("本机控制凭据已经失效。请重新配对后再关闭 Companion。");
      return;
    }
    setAiPanelBusy(true);
    setAiPanelError("");
    setAiPanelMessage("");
    try {
      await companionControlClient.shutdown(
        aiOverview.companion.controlUrl,
        companionHealth.instanceId,
        companionControlCredential,
      );
      setCompanionControlCredential(null);
  ...
  async function disconnectCompanion() {
    if (!aiOverview?.companion.controlUrl || !companionHealth || !companionControlCredential) {
      setAiPanelError("本机控制凭据已经失效。请重新配对后再断开 Companion。");
      return;
    }
    setAiPanelBusy(true);
    setAiPanelError("");
    setAiPanelMessage("");
    try {
      await companionControlClient.disconnect(
        aiOverview.companion.controlUrl,
        companionHealth.instanceId,
        companionControlCredential,
      );
  ...
  async function refreshCodexPairing() {
    ...
    try {
      ...
      if (nextPairing.status === "connected" && nextPairing.connectionId) {
        setCompanionHealth((current) => current
          ? { ...current, status: "connected", diagnostic: undefined }
          : current);
        ...
        setCompanionHealth((current) => current
          ? { ...current, status: "blocked", diagnostic: nextPairing.diagnostic }
          : current);
        ...
        setCompanionHealth((current) => current
          ? { ...current, status: "idle", diagnostic: undefined }
          : current);
  ...
  useEffect(() => {
    ...
    if (!companionControlClient.isLocalDevelopmentPage()) {
      setCompanionControlError("");
      return undefined;
    }
    let cancelled = false;
  ...
  useEffect(() => {
    ...
    setWorkspaceView(singleResult.status === "needs_review" ? "review" : "workbench");
  }, [singleResult?.status]);
  ...
  function clearStaleRunArtifacts() {
    setSingleResult(null);
    setJob(null);
    setError("");
    setStatus("idle");
  ...
  const reviewCount = reviewBlockerCount(singleResult, job);
  const formalArtifactReady = isFormalArtifactReady(singleResult, job);
  const activeStage =
          ...
          <div className="workspace-canvas">
            ...
            {workspaceView === "review" ? (
              <section className="review-workspace">
                ...
                {singleResult?.status === "needs_review" ? (
                  <SingleConversionReview
                    result={singleResult}
                    onReturnToInput={() => setWorkspaceView("input")}
                  />
                ) : job ? (
                  <Progress job={job} />
                ) : (
...
function DownloadPanel(props: {
  ...
  if (props.singleResult) {
    if (props.singleResult.status !== "accepted" || !props.singleResult.downloadUrl) {
      return (
        <div className="empty-state">
          <ExclamationTriangleIcon />
          <strong>这份 JSON 仍在人工核对，暂不提供下载</strong>
          <span>请查看核对问题，修改原文后重新转换。</span>
        </div>
      );
    }
    return (
...
function SingleConversionReview(props: {
  result: ConversionResult;
  onReturnToInput: () => void;
}) {
  const blockers = props.result.diagnostics.filter(
    (diagnostic) => diagnostic.severity === "warning" || diagnostic.severity === "error",
  );
  return (
    <section className="intake-review single-conversion-review">
      <div className="review-intro">
        <span>标准资料核对</span>
        <h3>预览已生成，但还不能当作正式 JSON 下载</h3>
        <p>系统保留了这次预览和问题说明；请修改来源后重新转换，不会覆盖任何正式文件。</p>
      </div>
      {blockers.map((diagnostic, index) => (
        <article className="review-creature" key={`${diagnostic.code}-${diagnostic.path}-${index}`}>
          <div className="intake-issue">
            <div className="issue-meta">
      ...
      <button className="primary-button review-submit" onClick={props.onReturnToInput}>
        返回修改原文
      </button>
    </section>
  );
...
export function reviewBlockerCount(
  singleResult: Pick<ConversionResult, "status" | "diagnostics"> | null,
  job: Pick<WebJob, "status" | "summary"> | null,
): number {
  if (singleResult?.status === "needs_review") {
    return Math.max(
      1,
      singleResult.diagnostics.filter(
        (diagnostic) => diagnostic.severity === "warning" || diagnostic.severity === "error",
      ).length,
    );
  }
...
export function isFormalArtifactReady(
  singleResult: Pick<ConversionResult, "status"> | null,
  job: Pick<WebJob, "status"> | null,
): boolean {
  return singleResult?.status === "accepted" || job?.status === "succeeded";
}
